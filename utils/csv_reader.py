...
f = open("./dataset/ddinter_downloads_code_V.csv")
i = 0
for line_read in f:
    ...
    if (i > 0):
        ...
        if (i % 1000 == 0):
            print("Test sampling : [" + drug_A + "] interact [" +
                  drug_B + "], with score : " + interaction_score)
    i = i + 1
...
for i in range(len(drug_A_list)):
    if (drug_A_list[i] in DtoS_dic and drug_B_list[i] in DtoS_dic):
        Output_data_list.append([DtoS_dic[drug_A_list[i]], DtoS_dic[drug_B_list[i]],
                                interaction_score_dict[interaction_score_list[i]]])
...
# writing to csv file
f = open("./dataset/DDI_data_V.txt", "w")

for i in range(len(Output_data_list)):
    f.write(Output_data_list[i][0] + " " + Output_data_list[i]
            [1] + " " + str(Output_data_list[i][2]) + "\n")
